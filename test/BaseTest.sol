...
import "contracts/Libra.sol";
import "contracts/LibraLibrary.sol";
...
abstract contract BaseTest is Test, TestOwner {
    ...
    function mintLibra(address[] memory _accounts, uint256[] memory _amounts) public {
        for (uint256 i = 0; i < _amounts.length; i++) {
            LIBRA.mint(_accounts[i], _amounts[i]);
        }
    }
    ...
    function deployPairFactoryAndRouter() public {
        ...
        lib = new LibraLibrary(address(router));
    }
