...
contract MinterTeamEmissions is BaseTest {
    ...
    function setUp() public {
        ...
        deployCoins();
        mintStables();
        uint256[] memory amountsLibra = new uint256[](2);
        amountsLibra[0] = 1e25;
        amountsLibra[1] = 1e25;
        mintLibra(owners, amountsLibra);
        team = new TestOwner();
        ...
        LIBRA.approve(address(router), TOKEN_1);
        ...
        address pair = router.pairFor(address(FRAX), address(LIBRA), false);

        LIBRA.approve(address(voter), 5 * TOKEN_100K);
        ...
        assertEq(LIBRA.balanceOf(address(minter)), 14 * TOKEN_1M);
        ...
        vm.roll(block.number + 1);
        before = LIBRA.balanceOf(address(owner));
        minter.update_period(); // initial period week 2
        after_ = LIBRA.balanceOf(address(owner));
    ...
    function testTeamEmissionsRate() public {
        ...
        vm.warp(block.timestamp + 86400 * 7);
        vm.roll(block.number + 1);
        uint256 beforeTeamSupply = LIBRA.balanceOf(address(team));
        uint256 weekly = minter.weekly_emission();
        uint256 growth = minter.calculate_growth(weekly);
        minter.update_period(); // new period
        uint256 afterTeamSupply = LIBRA.balanceOf(address(team));
        uint256 newTeamLibra = afterTeamSupply - beforeTeamSupply;
        assertEq(((weekly + growth + newTeamLibra) * 30) / 1000, newTeamLibra); // check 3% of new emissions to team

        vm.warp(block.timestamp + 86400 * 7);
        vm.roll(block.number + 1);
        beforeTeamSupply = LIBRA.balanceOf(address(team));
        weekly = minter.weekly_emission();
        growth = minter.calculate_growth(weekly);
        minter.update_period(); // new period
        afterTeamSupply = LIBRA.balanceOf(address(team));
        newTeamLibra = afterTeamSupply - beforeTeamSupply;
        assertEq(((weekly + growth + newTeamLibra) * 30) / 1000, newTeamLibra); // check 3% of new emissions to team

        // rate is right even when LIBRA is sent to Minter contract
        vm.warp(block.timestamp + 86400 * 7);
        vm.roll(block.number + 1);
        owner2.transfer(address(LIBRA), address(minter), 1e25);
        beforeTeamSupply = LIBRA.balanceOf(address(team));
        weekly = minter.weekly_emission();
        growth = minter.calculate_growth(weekly);
        minter.update_period(); // new period
    ...
    function testChangeTeamEmissionsRate() public {
        ...
        uint256 afterTeamSupply = LIBRA.balanceOf(address(team));
        uint256 newTeamLibra = afterTeamSupply - beforeTeamSupply;
        assertEq(((weekly + growth + newTeamLibra) * 50) / 1000, newTeamLibra); // check 5% of new emissions to team
    }
}
